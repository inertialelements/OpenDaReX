    ...
    * */
    ...
    public class BluetoothLeService extends Service {
        ...
        public static final String EXTRA_TX_VALUE =
                "com.example.bluetooth.le.EXTRA_TX_VALUE";

        // BEGIN - Added by GT Silicon - BEGIN //
        public final static UUID UUID_SERVER_TX_UART =
                UUID.fromString(SampleGattAttributes.SERVER_UART_tx);
        // END - Added by GT Silicon - END //
...

        {
            mGattCallback = new BluetoothGattCallback() {
                ...
                public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
                    Resources res = BluetoothLeService.this.getResources();
                    Log.w(TAG, "MTUres " + res);
                }
                // END - Added by GT Silicon - END //
            };
        }
        ...
        private void broadcastUpdate(final BluetoothGattCharacteristic characteristic) {
            ...
            // BEGIN - Added by GT Silicon - BEGIN //
            if (characteristic.getUuid().equals(UUID_SERVER_TX_UART)) {
                // DATA Parsing Hare
    ...
    // BEGIN - Added by GT Silicon - BEGIN //
        ...
        public static void setCharacteristicNotification(BluetoothGattCharacteristic characteristic,
            ...
            if (UUID_SERVER_TX_UART.equals(characteristic.getUuid())) {
    ...
    // END - Added by GT Silicon - END //
        ...
        public static int exchangeGattMtu(int mtu) {
            int retry = 5;
            boolean status = false;
            while (!status && retry > 0) {
                Log.w(TAG,"mtusize-"+mtu);
                try {
                    Thread.sleep(160);
